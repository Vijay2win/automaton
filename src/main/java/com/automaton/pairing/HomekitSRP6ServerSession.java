...
/**
 * This is a slightly modified version of the SRP6ServerSession class included with nimbus. The only change made for
 * homekit compatability is a change to the size of the b key. Homekit pairing fails if b is not 3072 bytes.
 * 
 * Stateful server-side Secure Remote Password (SRP-6a) authentication session. Handles the computing and storing of
 * SRP-6a variables between the protocol steps as well as timeouts.
 *
 * <p>
 * Usage:
 *
 ...
 * <li>If you wish to use custom routines for the server evidence message 'M1' and / or the client evidence message 'M2'
 * specify them at this point.
 * <li>Proceed to {@link #step1 step one} on receiving a valid user identity 'I' from the authenticating client. Respond
 * with the server public value 'B' and password salt 's'. If the SRP-6a crypto parameters 'N', 'g' and 'H' were not
 * agreed in advance between server and client append them to the response.
 * <li>Proceed to {@link #step2 step two} on receiving the public client value 'A' and evidence message 'M1'. If the
 * client credentials are valid signal success and return the server evidence message 'M2'. The established session key
 * 'S' may be {@link #getSessionKey retrieved} to encrypt further communication with the client. Else signal an
...
public class HomekitSRP6ServerSession extends SRP6Session {
    ...
    public static enum State {
        ...
        /**
         * The user identity 'I' is received from the client and the server has returned its public value 'B' based on
         * the matching password verifier 'v'. The session is ready to proceed to {@link #STEP_2}.
         */
        STEP_1,

        /**
         * The client public key 'A' and evidence message 'M1' are received and the server has replied with its own
         * evidence message 'M2'. The session is finished (authentication was successful or failed).
         */
        STEP_2
    }
     ...
     * @param timeout
     *            The SRP-6a authentication session timeout in seconds. If the authenticating counterparty (server or
     *            client) fails to respond within the specified time the session will be closed. If zero timeouts are
     *            disabled.
     */
    ...
    /**
     * Creates a new server-side SRP-6a authentication session and sets its state to {@link State#INIT}. Session
    ...
    /**
     * Increments this SRP-6a authentication session to {@link State#STEP_1} indicating a non-existing user identity 'I'
     * with mock (simulated) salt 's' and password verifier 'v' values.
     *
     * <p>
     * This method can be used to avoid informing the client at step one that the user identity is bad and throw instead
     * a guaranteed general "bad credentials" SRP-6a exception at step two.
     *
     * <p>
     * Argument origin:
     * 
     * <ul>
     * <li>From client: user identity 'I'.
     * <li>Simulated by server, preferably consistently for the specified identity 'I': salt 's' and password verifier
     * 'v' values.
     * </ul>
     *
     * @param userID
     ...
     *             If the session has timed out, the client public value 'A' is invalid or the user credentials are
     *             invalid.
     *
     * @throws IllegalStateException
